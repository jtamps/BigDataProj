...
import logging
import uuid
from llama_cpp import Llama # Added for Llama
...
def load_llama_model(model_path: str):
    ...
    if not os.path.exists(model_path):
        ...
        if not os.path.exists(expanded_path):
            logger.error(f"Llama model file not found at: {model_path} or {expanded_path}")
            st.error(f"LLM model file not found: {model_path}. Please download it and ensure the path is correct.")
            return None
        model_path = expanded_path # Use expanded path if it exists
        logger.info(f"Using expanded Llama model path: {model_path}")
    ...
    try:
        ...
        logger.info(f"Llama model loaded successfully from {model_path}. GPU layers requested: {N_GPU_LAYERS}")
        # Optional: Test completion removed for faster startup in app context, assume it works if loaded.
        return llm
    except Exception as e:
        logger.error(f"Error loading Llama model: {e}")
        st.error(f"Error loading LLM: {e}. Check logs for details. Ensure llama-cpp-python is compiled correctly (e.g. with Metal for M-series Macs).")
        return None
